...
- **Satellites preview with `tools/devwin.sh <lane>`, not `npm run dev`.** It pins the window to its own `.next-<lane>` (via `NEXT_DIST_DIR`) and its own port — world 3201, sprites 3202, play 3203, assets 3204 — so a preview can never touch the `.next` the hub deploys from. It refuses `hub` and refuses port 3200. A bare `npm run dev` is still banned because it defaults to both.
  - **A devwin is ON THE BOARD (2026-09-15).** `devwin.sh` registers its pid under `.coord/devwins/<lane>` while it runs and removes it on exit; `coord status` lists dev servers (live / ⚠ ORPHAN / DEAD), `coord release <lane>` stops the lane's devwin with the lane, and `coord reap` stops every devwin whose lane has no claim. **Why:** the spirits window wrapped 09-14, released its lane, and left `next dev` on :3205 for two days at ~600MB — the next hub build OOMed against it, the killed build wiped `.next/BUILD_ID` while prod kept serving from memory, and the lock then refused the rebuild over the leftover worker file. The claim said nobody was here; the process said otherwise; now the board joins the two. **Boot habit:** if `coord status` shows an orphan, `coord reap`. Test: `bash tools/coord-devwin.test.sh`.
