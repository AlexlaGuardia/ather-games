...
import bpy, bmesh, os, math, random, zlib, json
...
# ── ★ THE FAMILY DIALS (2026-09-22, jin/hub — the moonberry) ────────────────────────────────
# This file's own header promised the family: *"Reused for the whole bush FAMILY: change SEED /
# FRUIT_RGB / LEAF_RGB for a variant"* and *"a variant that needs a different silhouette edits
# that list"* — that last one meaning the `tiers` literal below, i.e. edit the shared script and
# break the other bush. So the list becomes an env override instead, and the shape is a dial like
# everything else. Unset = the sunfruit literal, byte-identical output.
#
# ★ CANON DREW THE MOONBERRY'S DIFFERENCES, NOT ME (`world/cuisine.md` › ★ ATHER FRUIT, ruled
# 2026-08-22). Sunfruit is *"a warm golden fruit"* — singular, few, big. Moonberry is *"cool blue
# BERRIES"* — plural, many, small. And they never stand side by side: sunfruit takes meadow and
# basin (open sun), moonberry woodland and shore (shade and damp), so the silhouettes can answer
# the ground they grow on. A sun plant is compact and dense; a shade plant is leggier and broader,
# because it is reaching.
NAME = os.environ.get('NAME', 'sunfruit-bush')
# TIERS: JSON list of [count, ring_radius, z, lump_radius] — the shape itself.
TIERS = os.environ.get('TIERS', '')
# FRUIT_DROOP: 0 = the sunfruit's outward-and-UP ray (fruit sits ON the crown). 1 = outward-and-
# DOWN (berries HANG under the shoulders). Canon's word for the moonberry is berries, and berries
# hang — it changes the silhouette, not just the count.
FRUIT_DROOP = float(os.environ.get('FRUIT_DROOP', '0'))
...
# ── ★★ A STABLE HASH, BECAUSE `hash()` IS NOT ONE (found 2026-09-22, jin/hub) ─────────────────
# The per-vertex jitter below used `hash((SEED, o.name, v.index, 'x'))`. **Python randomises
# `hash()` for str and bytes per PROCESS** (PYTHONHASHSEED), and both `o.name` and the axis letter
# are strings — so the jitter was reseeded on every run and `SEED` never controlled it at all.
#
# ⚠ WHAT THAT MEANT, AND IT IS WORSE THAN "the bush wobbles": **the same script at the same seed
# produced a DIFFERENT bush every run.** Caught by baking twice and diffing — two 360-triangle
# meshes with different vertices and a bounding box 3% apart in height. So the `.glb` in `public/`
# and the baked module could silently drift out of agreement, `/bushtest sculpt` (which loads the
# glb) could show a different plant from the one the wild pool draws, and — the real cost — **the
# exact bush Alex approved could not be regenerated by anyone, including this script.**
# A look-approval you cannot reproduce is not an approval, it is a screenshot.
#
# crc32 over a joined string: stable across processes, machines and Python versions, which is the
# whole requirement. This changes the sunfruit's shape ONCE, by a sub-texel jitter, and makes every
# bake after it reproducible forever.
def shash(*parts):
...
if TIERS:
    tiers = [tuple(t) for t in json.loads(TIERS)]
    print(f'TIERS override: {tiers}')
...
for tier_idx, (count, ring_r, z, r) in enumerate(tiers):
    for i in range(count):
        ...
        for v in bm.verts:
            v.co += Vector((
                (shash(SEED, o.name, v.index, 'x') % 100 - 50) / 900.0,
                (shash(SEED, o.name, v.index, 'y') % 100 - 50) / 900.0,
                (shash(SEED, o.name, v.index, 'z') % 100 - 50) / 900.0,
            ))
        bm.to_mesh(o.data)
        bm.free()
        lump_objs.append(o)
        if tier_idx >= FRUIT_FROM_TIER:
...
while len(fruit_objs) < N_FRUIT and tries < N_FRUIT * 80:
    ...
    # FRUIT_DROOP lerps the vertical bias from +0.35 (sits on the crown) to -0.30 (hangs under
    # the shoulder). The jitter band tilts with it, or a drooping berry still gets thrown upward
    # half the time and the two species come out the same plant again.
    vert = 0.35 * (1 - FRUIT_DROOP) + (-0.30) * FRUIT_DROOP
    d = outward * 0.7 + Vector((0, 0, 1)) * vert
    jz_lo = -0.3 * (1 - FRUIT_DROOP) + (-0.55) * FRUIT_DROOP
    jz_hi = 0.45 * (1 - FRUIT_DROOP) + 0.10 * FRUIT_DROOP
    d += Vector((random.uniform(-0.65, 0.65), random.uniform(-0.65, 0.65), random.uniform(jz_lo, jz_hi)))
    d.normalize()
