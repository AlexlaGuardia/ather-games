...
// The universe is derived in `voxel3d/item-universe.ts` so this script and the `dev/icons` page
// share ONE list. See that file's header for why a second copy here would go stale.
const sources = itemUniverse()
...
const rows: Row[] = [...sources.entries()]
  .map(([id, from]): Row => {
    ...
    // ⚠ 'mesh' JOINED THE CHAIN ON 2026-08-26 AND THIS LINE IS THE LESSON FROM 'cross' APPLIED
    // BEFORE IT COST ANYTHING. Widening a union leaves its consumers stale and quiet: when 'cross'
...
console.log(`\nuniverse joined from: ${UNIVERSE_SOURCES}`)
