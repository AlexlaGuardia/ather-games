...
> - **⚠ ALEX'S DIAL MOVED, AND THE ORACLE IS WHY: regen 15 → 17.** Four Moglins at Brack pulled the fastest clock to 9.3s, **under the 9.7s recovery**, breaking *"recovering the guard must never cost more than losing it"* — the rule set the morning before. Recovery is now 8.9s. **Smallest move that keeps the ordering; say the word and it goes back to 15 with the rule loosened instead.**
