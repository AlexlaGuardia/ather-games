...
> - **Parked:** stone has exactly ONE refine, so the cutter is thin — a second stone recipe would give it depth. A **Moglin at a station turns `runMs`** and later pulls input from a bound chest; still blocked on the `CANON_GAPS` entry.
> - **⏭️ Next:** playtest the mill AND the cutter together — 5s / 18s and the 4→3 yield are all first guesses in `STATIONS`. The tile art is procedural placeholder quality and wants Alex's eye.
