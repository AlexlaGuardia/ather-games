...
> - **★★ AND THE PICTURE SPLIT THE VERDICT IN TWO, WHICH NO ASSERT WOULD HAVE.** On a **highland** bank (`tp -1301 -233`) it is unmistakable — a cut passage running into the hillside, walls and floor receding, exactly the deliberate mouth `carve.ts` asked for. On a **dell** bank (`tp -2086 791`) the same feature is a shadowed notch you have to be told is there, because that ground is a **staircase of 1-block risers** and a mouth cut into steps reads as a gap between steps. **Same code, same asserts, two completely different features** — and the terraced case is the COMMON one, since meadow and dell are 47% of the world between them. ⚠ This is the cliff finding arriving a second time from the other side: the slope statistic said the banks are rare, the render says the ones that exist are mostly staircases.
