...
>
> **Next (ruled by Alex at wrap, 2026-08-07 + terrain session):** ① **Alex feel-pass the plains + greyfields on foot** (HUD names the biome underfoot); rule how far biome look goes (grey trees? crag bare rock?). ② **structure spawning on pads** — Alex floated *stronghold chance in greyfields*; pads exist now, `flatness`/`greyness` are the placement fields. **⛔ canon check first: "Moglin strongholds" are ruled WILDS features (shimmer-geography layer 3) — whether one may stand in garden-country greyfield is Magii's call; a generic ruin/hollow-structure may not be.** ③ re-aim **Greg's dialogue** to the seed (cozy register → **dispatch @raven**). ④ **grimoire**: real item or UI unlock — design call. ⑤ spawn cycle (buildable NOW on the grey band; creature ⛔ Magii — guns canon Q). ⑥ rinning block answer + crop blocks for farming. ⑦ class-1 structures (tutorial glade, Gloview, the 8 holds) — these want the pads too.
>
