...
>
> **Next:** ① **Alex look-calls the texture spike** — flat vs textured, 32 vs 64, at `/shimmer/voxel3d/tex`; fly back until the two patches stop differing, that distance IS the decision ② if textured wins, the **main/worker window** wires it in (promote the layer attribute through `attrs.ts` into the worker; optionally export a face id per quad from `greedy.ts` instead of deriving from normals — tidier, not required) ③ Alex re-rules world height **128 → 256** now the phone is gone (see below) ④ class-1/class-2 structure placement ⑤ carvers for Voranyx. *(Mesher spike DONE — `d018c68`, answer is 16. Texture spike DONE — see below. The 7-canvas composition is RETIRED — see the 08-06 biome ruling below.)*
>
