...
> - bubble **34** · bubble-wiring **38** · seam **55** · whole voxel core clean · canon 7 CLEAN · tsc unmoved. `slump` + `merge` fail identically on clean master (pre-existing). **⏭️ NEXT: Alex walks out and calls the three numbers** — the crown is the half he actually sees, best from the glade (the sightline the radius was chosen for) or `/tp -117 -500` for the door, **never the door's own column, which is inside the wall.**
