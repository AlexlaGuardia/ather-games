...
> **🪨 CHARACTER SLICE ③ — SCATTER: WHAT A LAND SHEDS (`d59d48e` + `d4ac82e`, pushed, canon **10 CLEAN**, `scatter.test.ts` **52** / 7 mutations each red on its own, full voxel suite green, tsc clean).** Rocks, deadfall and mushrooms per land, closing the third of Alex's four (#618). ⚠ **NOT DEPLOYED — see the worker note at the bottom.**
...
> - **⚠⚠ NOT DEPLOYED, AND THE FAILURE MODE IS SILENT.** `flora.ts` is in the **worker's import graph**, so the committed `voxel-gen.worker.<hash>.js` is now built from PRE-scatter source. **Deploy as-is and the app ships scatter's source, scatter's renderer, and a worker that generates none of it** — no error, no 404, just bare ground, which reads as *"the layer does nothing."* I did not commit a rebuilt worker on purpose: hub's uncommitted `character.ts` is in that same graph, so a rebuild now would inline their in-flight work into a committed bundle, invisible in the diff. **Hub commits findLands → `coord build` → the hashed artifact and `worker-url.ts` travel in that one commit** (check `git status` AFTER the build).
