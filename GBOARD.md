...
>
> **Next:** ① **fix play3d birth-after-reset — it is unreliable and flaps between runs.** `Shimmer3D` reads the save during render and re-persists it, so the ritual sometimes never opens; move birth into `play3d/page.tsx`'s boot gate the way `voxel3d/page.tsx` does. ② re-aim **Greg's dialogue** to the seed (cozy register → **dispatch @raven**, do not freehand). ③ decide whether the **grimoire** is a real inventory item or just the UI unlocking — it is in the canon bag but only exists as `/grimoire` + PartyPanel today. ④ **port step 2 — crafting** (`engine/crafting.ts` is zero-coupling), then the rest of the zero-coupling sixteen. ⑤ class-1 structures (tutorial glade, Gloview, the 8 holds). ⑥ then the three coupled systems: `spawn-board` (54 refs), `encounters` (25 → biome+depth), `player` (20). *(Mesher spike DONE — `d018c68`, answer is 16. Texture spike DONE — 64px texture arrays. The 7-canvas composition is RETIRED — see the 08-06 biome ruling below.)*
>
