...
> **V2 build order:** ① region fields for the anchored zones (terrain character + biome label as siblings, zone-aware trees/flora) ② vertical rebalance (sea/datum — Alex rules the split) ③ river-approach smoothing (kills the fragmented levee wall) ④ mist-patch encounters (site-pattern placement on pads, arena spar loop; roster waits on Magii) ⑤ hot-spring pools / hill shaping / canopy density per zone character.
>
> **Next (ruled by Alex at wrap, 2026-08-07 + terrain session):** ① **Alex NIGHT-WALK a greyfield: meet a Hollow, shoot it, run from one** — call the fear (speed/cap/hover are dials), the body's read, and whether the damage loop comes next. ② **Alex feel-pass: plains + greyfields + rivers (now wet-looking) + FIND A RUIN** (map has markers; HUD names the biome); rule the ruin's read, river width/depth feel, + how far biome look goes (grey trees? crag bare rock?). ③ re-aim **Greg's dialogue** to the seed (cozy register → **dispatch @raven**). ④ **grimoire**: real item or UI unlock — design call. ⑤ spawn cycle (buildable NOW on the grey band; creature ⛔ Magii — guns canon Q). ⑥ rinning block answer + crop blocks for farming. ⑦ class-1 structures (tutorial glade, Gloview, the 8 holds) — these want the pads too.
>
