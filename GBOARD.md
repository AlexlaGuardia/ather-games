...
> - [~] **Updraft** — **AIRS REWORK shipped 2026-07-11 (`25a135d`).** Seedfall playbook applied to its canon sibling (`CANON/game/updraft.md`): the endless climb cycles four looping airs — Open Current(wide) → Gate-Reach(tight) → **Rising Thermal**(gravity eases, a real lift) → **Churn**(void-gate gaps DRIFT) — each with palette + gate colour + label; endless ramp (scroll 156→210) + per-air gaps; START first-gate timing fixed (`launch()`→VW+40). Proven at sim level (thermal 87→35px, churn ±46, all 4 airs) + drift-gate 5/5 + live 200. **NEXT: Alex device feel** — knobs atop `lib/updraft.ts` (`AIR_LEN`/`THERMAL_G`/`CHURN_AMP`/`CHURN_RATE`/ramp). Matched pair w/ Seedfall (the climb + the fall).
