...
function CatalogCard({ g, favs, onToggle }: { g: GameEntry; favs: string[]; onToggle: (id: string) => void }) {
  ...
  const cls =
    "group relative rounded-xl border border-white/[0.06] bg-[#12121e]/70 p-5 transition-all overflow-hidden min-h-[140px]";
  ...
  if (soon) {
    return <div className={`${cls} opacity-50 cursor-default`}>{body}</div>;
  }
  return (
