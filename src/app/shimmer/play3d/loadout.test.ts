...
{
  reset()
  const fresh = loadLoadout(OWNED)
  ok(fresh.length === CAST_SLOTS.length, 'a loadout always has exactly one entry per slot')
  ok(JSON.stringify(fresh) === JSON.stringify(defaultLoadout(OWNED)),
...
}
{
  ...
  const legal = defaultLoadout(OWNED)
  const bound = legal.findIndex((m) => m !== null)
  if (bound >= 0) {
    saveLoadout(legal)
    const back = loadLoadout([])   // keeper now holds no runes at all
...
}
{
  ...
  ok(JSON.stringify(loadLoadout(OWNED)) === JSON.stringify(defaultLoadout(OWNED)),
...
// ── setSlot ───────────────────────────────────────────────────────────────────────────────────
{
  ...
  const kit = defaultLoadout(OWNED)
  const move = kit.find((m) => m !== null)!
  const slot = kit.indexOf(move)

  ok(setSlot(OWNED, empty, slot, move)[slot] === move, 'a legal bind lands in its slot')
  ok(setSlot(OWNED, empty, slot, 'no-such-move')[slot] === null,
     'an illegal bind is refused — the caller cannot produce an invalid loadout')
  ok(setSlot(OWNED, empty, 99, move).every((s) => s === null), 'an out-of-range slot changes nothing')
  ok(setSlot(OWNED, empty, -1, move).every((s) => s === null), 'and so does a negative one')

  const held = setSlot(OWNED, empty, slot, move)
  ok(setSlot(OWNED, held, slot, null)[slot] === null, 'a slot can be cleared')
...
// ── the absent-localStorage path (the shape this runs in outside a browser) ───────────────────
{
  ...
  let threw = false
  let kit: Loadout = []
  try { kit = loadLoadout(OWNED) } catch { threw = true }
  ok(!threw, '★ no localStorage must not throw — private mode is a keeper, not a crash')
  ok(JSON.stringify(kit) === JSON.stringify(defaultLoadout(OWNED)), 'and they still get a kit')
  let saveThrew = false
