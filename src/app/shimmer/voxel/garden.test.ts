...
import { PLOT_ALLOWANCE, plotsAllowed, nextAllowance, placeBedBlocker, plotRefusalLine, countBeds } from './garden'
...
// ── 5. THE COUNT IS DERIVED FROM THE EDIT LOG ──────────────────────────────────────────────────
{
  ...
  ok(countBeds([], BED) === 0, 'no columns, no beds')
  ok(countBeds([col(STONE, STONE)], BED) === 0, 'edits that are not beds do not count')
  ok(countBeds([col(BED, STONE, BED)], BED) === 2, 'two beds in one column')
  ok(countBeds([col(BED), col(BED, BED)], BED) === 3, '★ beds are counted across columns, not per column')
  ...
  ok(countBeds([col(STONE)], BED) === 0, '★★ a bed replaced by something else is no longer a bed')
  ...
  ok(countBeds([col(1, 2, 3, 4, 5)], BED) === 0, '★ only the bed material counts, not any edit')
}
