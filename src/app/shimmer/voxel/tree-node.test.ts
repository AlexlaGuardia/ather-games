...
import { TREE_NODES, treeId, isStanding, fellTree, logItem, saplingItem, speciesMissingNode } from './tree-node'
...
// ── 3. the payout itself ─────────────────────────────────────────────────────────────────────
{
  ...
  ok(unlucky.xp === lucky.xp && lucky.xp === 20, 'XP is for the act, not the luck')
...
{
  const xp = SPECIES.map(s => TREE_NODES[s.id]?.xp ?? 0)
