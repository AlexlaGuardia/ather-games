...
console.log('\n── 7. the host and the worker are wired ──')
{
  ...
  ok((codeOnly(host).match(/withLitter\(plotForTier\(/g) || []).length === 5, `every host plot config goes through withLitter (${(codeOnly(host).match(/withLitter\(plotForTier\(/g) || []).length} of 4)`)
