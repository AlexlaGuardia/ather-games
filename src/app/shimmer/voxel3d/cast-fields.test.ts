...
import { HOLLOW_HOVER, HOLLOW_RADIUS } from './hollows'
...
// ── 1. ★ THE OVERLAP THE WHOLE FEATURE RESTS ON ────────────────────────────────────────────────
{
  ...
  // A Hollow drifting across the field, standing on the same ground the cast landed on.
  const hollowY = GROUND + HOLLOW_HOVER
  chk('★ a Hollow hovering on the field\'s own ground line IS inside the slab',
    containsVolume(f, 10, hollowY, 10), `hollow y=${hollowY} vs slab ${f.y - FIELD_UNDERBITE}..${f.y + f.height}`)

  // The margins, stated rather than assumed — so a retune that eats them fails HERE and says why.
  const headroom = (f.y + f.height) - hollowY
  const underroom = hollowY - (f.y - FIELD_UNDERBITE)
  chk('...with real headroom above it, not by a hair', headroom > 1, `headroom ${headroom.toFixed(2)}`)
  chk('...and real room below it', underroom > 1, `underroom ${underroom.toFixed(2)}`)
  ...
  chk('the host tests a Hollow\'s CENTRE, so its body radius is bite it gets for free',
    !containsVolume(f, 10 + 3.2 + 0.01, hollowY, 10) && HOLLOW_RADIUS > 0)
}
...
// ── 2. verticality is the reason the slab exists ────────────────────────────────────────────────
{
  ...
  chk('a round at body height IS eaten (Firewall is cover)',
    blocksShotAtVolume([f], 0, GROUND + HOLLOW_HOVER, 0))
}
...
// floats — this asserts the intent so the regression has a name.
{
  ...
  chk('a field cast downhill bites at the LOWER ground line',
    containsVolume(f, 0, GROUND - downhill + HOLLOW_HOVER, 0))
  chk('...and not up at the caster\'s own feet', !containsVolume(f, 0, GROUND + HOLLOW_HOVER, 0))
}
