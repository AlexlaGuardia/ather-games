...
export default function VoxelWorld() {
  ...
  return (
    <div className="fixed inset-0 bg-[#0b0d14]">
      ...
      {(bagOpen || openChest) && (
        <BagPanel inv={inv} spiritIndex={spiritIndex} chest={openChest} tick={craftTick} sel={sel} castKeys={CAST_KEYS}
...
function World({ bindings, pad, inv, toolTier, toolSkill, vitals, mana, buffs, selItem, selSlot, weaponDrawn, weaponIdx, onAmmo, onStats, onPerf, onProfile, onSay, onContextLost, runeTick, onVesselFound, onPos, onLook, onInvChange, worker, incoming, inflight, settings, rot, tools, skills, onSkill, onLevel, onTool, tutorial, onQuestEvent, onNearGreg, onNearFolk, onNearTable, onCollarNear, cmdOut, mistLedger, onNearMist, onDiscover, sparring, pot, plotCfg, plotTier, litterFrom, spiritIndex, party, snapOut, space, clusterOut, lookOut, ctxLostOut, onOpenChest, onOpenStation, onOpenWaymark, onOpenGardens, onOpenBrew, onCauldronPlaced, brewings, uiOpen, uiSteps, owner, foesOut, pressOut, waterOut, castOut, tremorOut, hourLight }: {
  ...
  // ★ THE MODELLED STATIONS (2026-09-15) — the mesher emits no cube for a station id; this draws its
  ...
  const plantProbe = useCallback((fx: number, fz: number) => {
    ...
    const kind = m === MAT.TUFT ? FLORA.TUFT : m === MAT.TALL_GRASS ? FLORA.TALL
      ...
      : isForage(m) ? forageKind(m) : flowerForm(fx, fz, SEED)
    ...
    // A log cell's variant is its place in its RUN plus the run's shared roll (sculpt queue ④), read
    // off the same neighbour voxels the axis just was — so a trunk is one radius and has two ends.
    const variant = kind === FLORA.DEADFALL
      ? deadfallVariant(fx, fz, alongX, SEED, (ax, az) => voxel(ax, y + 1, az) === MAT.DEADFALL)
      : plantVariant(fx, fz, SEED, kind)
    return { y: gy, kind, variant, mat: m, ground, alongX }
  }, [voxel])
