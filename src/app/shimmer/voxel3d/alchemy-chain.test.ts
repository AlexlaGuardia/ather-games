...
// ── §4 the job maths ────────────────────────────────────────────────────────────────────────────
{
  ...
  ok(alchemyMaxRuns(fin, (id) => id === 'raw_mana_shard' ? 7 : id === 'glow_moss' || id === 'violetbloom_petal' ? 9 : 0, 999) === 1, '§4 and by the inputs (7 shards at 4 a run = 1; the moss and the petal are plentiful)')
}
