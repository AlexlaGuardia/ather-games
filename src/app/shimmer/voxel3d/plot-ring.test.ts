...
import { ringCap, inView, pickHome, reflowRing, DEFAULT_RING, type RingSlot, type Keeper } from './plot-ring'
...
// ── 3. pickHome — in the band, out of view, and honest about failing ─────────────────────────────
{
  ...
  let placed = 0, tooNear = 0, tooFar = 0, seen = 0
  for (let n = 0; n < 400; n++) {
    ...
    if (d > D.nearMax + 1e-9) tooFar++
    if (inView(k, h.x, h.z, D)) seen++
  }
  ok(placed > 300, `a permissive world must place nearly every time — placed ${placed}/400`)
  ok(tooNear === 0 && tooFar === 0, `every corner must land inside the band — ${tooNear} near, ${tooFar} far`)
...
// ── 4. reflowRing — the cast ─────────────────────────────────────────────────────────────────────
{
  ...
  // ⚠ THE FIRST VERSION OF THIS FIXTURE SAT AT `farOut + 40` = 130, WHICH IS PAST `sight` 120 — so
  // it was not in view at all and the oracle correctly recycled it. The window between `farOut` and
  // `sight` is the only place this rule can be observed, and a fixture outside it tests nothing.
  const watched: RingSlot[] = [{ id: 'a', hx: (D.farOut + D.sight) / 2, hz: 0, gen: 1 }]
  const kept = reflowRing(watched, keeper(0, 0, 0), ids, 3, 42, 4, no, D)
  ok(kept.some(s => s.id === 'a' && s.hx === watched[0].hx), '★★ a resident the keeper is LOOKING AT must never be moved, however far away')

  // ...and the same resident behind the keeper is recycled.
  const behind: RingSlot[] = [{ id: 'a', hx: -(D.farOut + 40), hz: 0, gen: 1 }]
  const recycled = reflowRing(behind, keeper(0, 0, 0), ids, 3, 42, 5, no, D)
  ok(!recycled.some(s => s.hx === behind[0].hx), '★ a resident far behind you must give up its corner')

  // a freed spirit is eligible again at once, at a NEW corner with a bumped generation.
  const back = reflowRing(behind, keeper(0, 0, 0), ['a'], 3, 42, 6, yes, D)
  ...
  ok(!!home && home.hx !== behind[0].hx, 'it must come back at a new corner, not the one it just left')
  ok(!!home && home.gen > behind[0].gen, '★ the generation must bump, or the walk it derives never changes')
