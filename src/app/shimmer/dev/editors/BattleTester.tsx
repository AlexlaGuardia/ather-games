...
import { SPECIES_ART } from '../../sprites/registry'

// Derived from `sprites/registry.ts`, not restated — the third of the four hand-kept copies.
// ⚠ Kept as a plain id -> anims record because it is handed straight to <BattleScene sprites={...}>,
// which indexes it by species id. Shape unchanged; only where the entries come from has changed.
const SPRITE_MAP: Record<string, Record<string, SpriteAnim>> = Object.fromEntries(
  Object.entries(SPECIES_ART).map(([id, art]) => [id, art.anims]),
)
