...
function Seam({ space, dist, frozen }: { space: Space; dist: number; frozen: boolean }) {
  ...
  const pass = useMemo(() => createSeamShimmer(SEED, WILDS_BUBBLE), [])
  const anchor = useMemo(() => (space === 'plot' ? plotSeamAnchor(SEED) : wildsSeamAnchor(SEED, WILDS_BUBBLE)), [space])
 ...
 */
function initial(): { space: Space; dist: number; frozen: boolean } {
...
export default function SeamPreview() {
  ...
  useEffect(() => {
    const q = initial()
    setSpace(q.space); setDist(q.dist); setFrozen(q.frozen)
  }, [])
  ...
  return (
    <div ref={wrap} style={{ position: 'fixed', inset: 0, background: '#0b0d10' }}>
      <Canvas camera={{ fov: 70, near: 0.1, far: 4000 }}>
        <Seam space={space} dist={dist} frozen={frozen} />
      </Canvas>
      <div style={{
