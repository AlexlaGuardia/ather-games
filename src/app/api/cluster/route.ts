...
export function GET(req: NextRequest) {
  ...
  if (!user_id) return NextResponse.json({ cluster: null, offers: [], quarter: null, invites: [] }, noStore)
  // `quarter` = where the ASKER stands, so their client can frame the cluster around their own fold.
  return NextResponse.json({ cluster: getCluster(user_id), offers: offersFor(user_id), quarter: myQuarter(user_id), invites: invitesFor(user_id) }, noStore)
}
